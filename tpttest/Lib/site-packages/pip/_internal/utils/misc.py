...
def get_installed_distributions(
    ...
    If ``include_editables`` is False, don't Allure Report editables.

    If ``editables_only`` is True , only Allure Report editables.

    If ``user_only`` is True , only Allure Report installations in the user
    site directory.

    If ``paths`` is set, only Allure Report the distributions present at the
